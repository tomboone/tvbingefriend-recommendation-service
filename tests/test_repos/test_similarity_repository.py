...
class TestStoreSimilarities:
    ...
    def test_store_similarities_deletes_existing_records(self, similarity_repository, test_db_session):
        ...
        similar_shows = [
            {'similar_show_id': 2, 'similarity_score': 0.8}
        ]
        ...
        old = test_db_session.query(ShowSimilarity).filter_by(
            show_id=1, similar_show_id=99
        ).first()
        assert old is None

        # New record should exist
        new = test_db_session.query(ShowSimilarity).filter_by(
    ...
    def test_store_similarities_with_batch_size(self, similarity_repository, test_db_session):
        ...
        # Arrange
        similar_shows = [
            {'similar_show_id': i, 'similarity_score': 0.8} for i in range(2, 12)
        ]
    ...
    def test_store_similarities_without_optional_scores(self, similarity_repository, test_db_session):
...
class TestBulkStoreAllSimilarities:
    ...
    def test_bulk_store_all_similarities_creates_all_records(self, similarity_repository, test_db_session):
    ...
    def test_bulk_store_all_similarities_clears_existing_when_requested(
        ...
        all_similarities = {
            1: [{'similar_show_id': 2, 'similarity_score': 0.8}]
        }
    ...
    def test_bulk_store_all_similarities_with_batch_size(self, similarity_repository, test_db_session):
...
class TestGetSimilarShows:
    ...
    def test_get_similar_shows_filters_by_min_similarity(
        ...
        # Act
        results = similarity_repository.get_similar_shows(
            show_id=1, n=10, min_similarity=0.6
        )
    ...
    def test_get_similar_shows_returns_empty_list_when_none_found(
...
class TestGetSimilarShowsWithMetadata:
    ...
    def test_get_similar_shows_with_metadata_returns_complete_info(
        ...
        similarity = ShowSimilarity(
            show_id=1, similar_show_id=2, similarity_score=0.85,
            genre_score=0.9, text_score=0.8, metadata_score=0.85
        )
...
class TestCountSimilarities:
    ...
    def test_count_similarities_returns_total_count(
        self, similarity_repository, test_db_session
    ):
    ...
    def test_count_similarities_for_specific_show(
        self, similarity_repository, test_db_session
    ):
...
class TestDeleteSimilarities:
    ...
    def test_delete_similarities_removes_all_for_show(
        self, similarity_repository, test_db_session
    ):
    ...
    def test_delete_similarities_returns_zero_when_none_exist(
        self, similarity_repository
    ):
