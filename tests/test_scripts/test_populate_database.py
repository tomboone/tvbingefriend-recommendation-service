...
class TestComputeAndStoreSimilarities:
    ...
    def test_compute_and_store_similarities_success(
        self, mock_np_load, mock_load_npz, mock_read_csv,
        mock_show_sim, mock_session_local, mock_repo_class,
        mock_computer_class, temp_data_dir
    ):
...
class TestMain:
    ...
    def test_main_success_with_defaults(
        self, mock_parse_args, mock_service_class, mock_load_sync,
        mock_compute, mock_verify
    ):
    ...
    @patch('argparse.ArgumentParser.parse_args')
    @patch('sys.exit')
    def test_main_handles_service_init_error(
        self, mock_exit, mock_parse_args, mock_service_class
    ):
