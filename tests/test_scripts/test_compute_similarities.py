...
class TestComputeSimilarities:
    ...
    def test_compute_similarities_with_default_weights(self):
        ...
        with patch('scripts.compute_similarities.SimilarityComputer') as MockComputer:
            ...
            mock_computer.get_similarity_statistics.return_value = {
                'mean': 0.33, 'std': 0.47, 'min': 0.0, 'max': 1.0, 'median': 0.0
            }

            result = compute_similarities(features=features)
...
class TestMain:
    ...
    def test_main_success_with_defaults(
        self, mock_parse_args, mock_load, mock_compute, mock_save
    ):
    ...
    @patch('scripts.compute_similarities.load_features')
    @patch('argparse.ArgumentParser.parse_args')
    def test_main_with_save_enabled(
        self, mock_parse_args, mock_load, mock_compute, mock_save
    ):
    ...
    def test_main_with_custom_weights(
        ...
        mock_compute.assert_called_once_with(
            features=mock_load.return_value,
            genre_weight=0.3,
            text_weight=0.6,
            metadata_weight=0.1
        )
    ...
    def test_main_handles_compute_error(
        self, mock_exit, mock_parse_args, mock_load, mock_compute
    ):
