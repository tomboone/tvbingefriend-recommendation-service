...
def load_and_sync_metadata(service: ContentBasedRecommendationService, input_dir: Path) -> int:
    """
...
def compute_and_store_similarities(
    ...
    # Note: SimilarityComputer initialized but not used directly here
    # Similarity computation done inline for memory efficiency
    _ = SimilarityComputer(
    ...
    for idx, show_id in enumerate(show_ids):
        ...
        # Hybrid similarity
        hybrid_sim = (
            genre_weight * genre_sim + text_weight * text_sim + metadata_weight * metadata_sim
        )
        ...
        if ((idx + 1) % batch_size) == 0 or (idx + 1) == len(show_ids):
            if all_similarities:
                ...
                batch_records = repo.bulk_store_all_similarities(
                    all_similarities, clear_existing=False
                )
                total_records_stored += batch_records
                logger.info(
                    f"  Processed {idx + 1}/{len(show_ids)} shows... (stored {total_records_stored} similarity records)"
                )
...
def verify_recommendations(
    service: ContentBasedRecommendationService, metadata_path: Path, num_tests: int = 3
):
    """
...
def main():
    ...
    parser.add_argument(
        "--top-n", type=int, default=20, help="Top N similar shows to store per show (default: 20)"
    )
    parser.add_argument(
        "--min-similarity",
        type=float,
    ...
    parser.add_argument("--skip-test", action="store_true", help="Skip recommendation testing")
    parser.add_argument(
    ...
    try:
        ...
        if not args.skip_test:
            verify_recommendations(
                service=service, metadata_path=input_dir / "shows_metadata.csv", num_tests=3
            )
        else:
