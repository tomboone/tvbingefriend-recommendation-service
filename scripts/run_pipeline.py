...
def run_migrations() -> bool:
    ...
    try:
        ...
        subprocess.run(cmd, check=True, capture_output=False, cwd=str(project_root))
        logger.info("=" * 70)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run the complete recommendation data pipeline")
    ...
    if invalid_steps:
        ...
        logger.error(
            f"Valid steps: {valid_steps} (Note: 'similarities' step removed - now computed in 'populate' step)"
        )
        sys.exit(1)
    ...
    if "populate" in steps:
        ...
        populate_args = [
            ...
            args.data_dir,
            "--genre-weight",
            str(args.genre_weight),
            "--text-weight",
            str(args.text_weight),
            "--metadata-weight",
            str(args.metadata_weight),
            "--top-n",
            str(args.top_n),
            "--min-similarity",
